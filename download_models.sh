...
BGE_RERANKER_DIR="${HOST_MODELS_DIR}/${BGE_RERANKER_MODEL_PATH:-bge}"

# Get default model names from environment or use defaults
DEFAULT_EMBEDDING_MODEL_NAME=${DEFAULT_EMBEDDING_MODEL:-"bge-m3"}
...
DEFAULT_BGE_RERANKER_MODEL_NAME=${DEFAULT_BGE_RERANKER_MODEL:-"bge-reranker-large"}

# Model identifiers (Hugging Face)
BGE_MODEL_ID=${HF_EMBEDDING_MODEL:-"BAAI/bge-m3"}
...
BGE_RERANKER_MODEL_ID=${HF_BGE_RERANKER_MODEL:-"BAAI/bge-reranker-large"}
...
BGE_RERANKER_MODEL_DIR="${BGE_RERANKER_DIR}/${DEFAULT_BGE_RERANKER_MODEL_NAME}"

# Create directories if they don't exist
mkdir -p "${EMBEDDING_MODEL_DIR}" "${COLBERT_MODEL_DIR}" "${LLM_MODEL_DIR}" "${WHISPER_MODEL_DIR}" "${BGE_RERANKER_MODEL_DIR}"
...
download_if_empty "$BGE_RERANKER_MODEL_ID" "$BGE_RERANKER_MODEL_DIR" "BGE Reranker model"
...
echo "- BGE Reranker model: ${BGE_RERANKER_MODEL_DIR}"
echo ""
...
echo "- BGE Reranker model: ${CONTAINER_MODELS_DIR}/${BGE_RERANKER_MODEL_PATH}/${DEFAULT_BGE_RERANKER_MODEL_NAME}"