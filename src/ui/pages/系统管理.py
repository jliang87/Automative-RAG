"""
System Dashboard page - Advanced monitoring for system administrators
"""
...
# Add the enhanced worker status component
from src.ui.enhanced_worker_status import enhanced_worker_status
from src.ui.task_progress_visualization import render_priority_queue_visualization
...
def render_system_dashboard():
        ...
        if workers:
            ...
            for worker_id, info in workers.items():
                ...
                worker_data.append({
                    ...
                    "类型": worker_type,
                    "状态": status,
                    "最后心跳": f"{heartbeat_age:.1f}秒前",
                    "启动时间": "未知"  # Could be calculated if first heartbeat is stored
                })
            ...
            for worker in worker_data:
                ...
                if wtype not in worker_types:
                    worker_types[wtype] = []
                worker_types[wtype].append(worker)

            # Display workers by type
            for worker_type, workers in worker_types.items():
                with st.expander(f"{worker_type} Workers ({len(workers)})", expanded=True):
                    # Create DataFrame
                    df = pd.DataFrame(workers)
                    st.dataframe(df, hide_index=True, use_container_width=True)

                    # Add restart button
                    if st.button(f"重启所有 {worker_type} Workers", key=f"restart_{worker_type}"):
    ...
    with tab3:
        ...
        if priority_queue_status:
            ...
            render_priority_queue_visualization(priority_queue_status)
