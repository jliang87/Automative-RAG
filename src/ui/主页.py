...
st.subheader("视频处理引擎")
st.write("""
本系统使用统一的视频处理引擎，具有以下特点：
- **统一的 Whisper AI 转录**：所有视频平台（YouTube、Bilibili 等）均使用相同的高质量转录引擎
- **GPU 加速**：利用 CUDA 实现更快的转录速度
- **多语言支持**：自动检测并支持多种语言的视频内容
- **自动平台检测**：根据 URL 自动识别视频平台
- **元数据提取**：从视频信息中自动提取汽车相关元数据
""")
...
* **统一视频处理**: **使用 Whisper 为所有平台提供一致的高质量转录**
...
1. **数据导入**：进入"数据导入"选项卡，上传汽车相关数据：
   - 视频：支持 YouTube、Bilibili 等平台的汽车评测与展示视频
